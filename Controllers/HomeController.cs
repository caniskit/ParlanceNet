...
namespace ParlanceNet.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            try
            {
                ...
                if (user != null)
                {
                    ...
                    var allPost = RefreshPost();
                    await Task.WhenAll(allPost);
                    model.allPost = allPost.Result.allPost;
                    return View(model);
                }
            }
            catch (Exception ex)
        ...
        public async Task<GlobalViewModel> RefreshPost()
        {
            string name;
        ...
        public ActionResult LikePost(int postId)
        {
            ...
            {
                sessionId = int.Parse(Session["UserId"].ToString());
            }
            else
            {
                ViewBag.Error = "Session Timeout..";
            ...
            if (userService.likePost(postId, sessionId) > 0)
            {
                var likeCount = postService.getLikeUsersPostById(postId).Count;
